...
var MM_APITestViewJS = MM_Core.extend({
	...
	chooseProvider: function(){
		var form = mmJQuery("#provider").val();
		mmJQuery("#method").find('option').remove().end();
		if(form=="paypal"){
			mmJQuery("#method").append("<option value=''>Choose Method</option>");
			mmJQuery("#method").append("<option value='checkout'>checkout</option>");
		}
		else if(form=="clickbank"){
			mmJQuery("#method").append("<option value=''>Choose Method</option>");
			mmJQuery("#method").append("<option value='checkout'>checkout</option>");
		}
		else{
			alert("System does not recognize this response");
		}
	},
	
    chooseForm: function(){
		var form = mmJQuery("#provider").val()+"_"+mmJQuery("#method").val();
		var values = {};
		values.form = form;
...
var mmjs = new MM_APITestViewJS("MM_IPNTestView", "IPN Test");