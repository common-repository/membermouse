...
class Parser
{
    ...
    {
        $nextRun = ($fromTime instanceof DateTime) ? $fromTime : new DateTime($fromTime ?$fromTime: 'now');
        ...
        $i = 0;
echo "getNextRunDate() : ".$nextRun->format('Y-m-d H:i:s')."\n";
    ...
    public function isDue($lastRun = 'now', $currentTime = 'now')
    {
    	...
    	$nextRun1 = $this->getNextRunDate($lastRun, $currentTime);
    	echo $currentTime." [currentTime] \n";
    	$nextRun2 = $this->getNextRunDate('now', $currentTime);
    	$from = $nextRun1->format('Y-m-d H:i:s');
    	$to = $nextRun2->format('Y-m-d H:i:s');
    	echo $from." != {$to}\n";
    	echo "\n\n**END isDue()**\n";
        return ( $nextRun1 != $nextRun2);
    }
}
?>